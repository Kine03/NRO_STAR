...
public class TopBanDoKhoBau {
    ...
    public void load() {
        ...
        try ( Connection con = DBService.gI().getConnectionForGetPlayer();  PreparedStatement ps = con.prepareStatement("SELECT *,CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBDKB, ',', 1), '[', -1) AS UNSIGNED) AS part1, CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichBDKB, ',', -1), ']', 1) AS UNSIGNED) AS part2 FROM clan_sv1 ORDER BY part1 DESC, part2 ASC LIMIT 100;");
