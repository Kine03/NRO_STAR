...
//    }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                top.player.head = rs.getShort("head");
                    ...
                    if (top.player.inventory.itemsBody.size() == 14) {
                        top.player.inventory.itemsBody.add(ItemService.gI().createItemNull());
                    }
                    dataArray.clear();
                    dataObject.clear();
    ...
    public static List<SieuHangModel> GetTop(int player_id) {
        List<SieuHangModel> result = new ArrayList<>();
        Connection con = null;
        CallableStatement ps = null;
        try {
