...
public class TopKillWhisManager {
    ...
    try (Connection con = DBService.gI().getConnectionForGetPlayer();  
         PreparedStatement ps = con.prepareStatement("SELECT * FROM player WHERE CAST(player.levelKillWhis AS UNSIGNED) > 0 ORDER BY CAST(player.levelKillWhis AS UNSIGNED) DESC, player.timeKillWhis ASC LIMIT 100;");
