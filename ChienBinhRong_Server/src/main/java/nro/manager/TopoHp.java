...
public class TopoHp {
   @Getter
    ...
    private static final TopoHp INSTANCE = new TopoHp();

    public static TopoHp getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DBService.gI().getConnectionForGetPlayer();
             PreparedStatement ps = con.prepareStatement("SELECT id, name, head, gender, point_hanhtrang, items_body, CAST(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(point_hanhtrang, ',', -3), '[', -1), ']', '') AS UNSIGNED) AS point_hanhtrang_first_element FROM player ORDER BY point_hanhtrang_first_element DESC LIMIT 100;");
