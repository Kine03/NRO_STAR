...
public abstract class Boss extends Player implements BossInterface {
    ...
    public void update() {
        ...
        try {
            ...
            if (Util.canDoWithTime(lastTimeUpdate, 60000 + Util.nextInt(9000, 15000))) {
                if (timeAppear >= TIME_RESEND_LOCATION) {
                    if (this.zone != null && !MapService.gI().isMapBanDoKhoBau(this.zone.map.mapId)
                            ...
                            && !(this instanceof BossZenoWar) && !(this instanceof BossMabuWar)
                            && !(this instanceof Broly)
                            && !(this instanceof Boss_ThanMeo)
                            && !(this instanceof Boss_Yanjiro) && !(this instanceof BossYardart)) {
    ...
    public void joinMap() {
        ...
        if (this.zone != null) {
            ...
            ServerNotify.gI().notify("Boss " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName);
            System.out.println("Boss: " + this.name + " xuất hiện mapId: " + this.zone.map.mapId + " Khu: " + this.zone.zoneId);
    ...
    public void generalRewards(Player player) {//Hmmmmm....phẩn thưởng chung (boss nào cũng rớt - boss phó bản)
        if (player != null) {
            ...
            if (Util.isTrue(40, 100)) {
                itemMap = new ItemMap(this.zone, NRs[randomNR], 1, x, y, player.id);
            }
