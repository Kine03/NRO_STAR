    ...
    public int injured(Player plAtt, int damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
            if (plAtt != null) {
                ...
                // Gọi super trước để máu bị trừ
                int realDamage = super.injured(plAtt, damage, piercing, isMobAttack);

                // Sau đó mới gọi resetPoint, để có thể tăng lại máu
                resetPoint(realDamage);
                addPlayerAttack(plAtt);
//                System.out.println("Broly HPG sau khi tăng: " + this.nPoint.hpg + ", dame: " + this.nPoint.dameg);
            }
    ...
    private void resetPoint(int damageInjured) {
        if (this.nPoint.hpg < MAX_HP && this.countResetPoint++ >= maxCountResetPoint) {
            ...
            // Cập nhật máu hiện tại bằng máu gốc mới
//            this.nPoint.hp = this.nPoint.hpg;
//            PlayerService.gI().sendInfoHp(this);
//            System.out.println("Broly HP sau khi tăng: " + this.nPoint.hp + ", dame: " + this.nPoint.dameg);
    ...
    @Override
    public void die() {
        this.secondTimeRestToNextTimeAppear = Util.nextInt(60, 120);
        super.die();
    }
