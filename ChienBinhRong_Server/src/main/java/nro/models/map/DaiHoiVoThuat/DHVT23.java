    ...
    public void update() {
        if (time > 0) {
            ...
            if (!player.isDie() && player != null && player.zone != null) {
                ...
                if (player.location.y > 336 && time > 10) {
                    leave();
                }
            } else {
                endChallenge();
        ...
        if (timeWait > 0) {
            switch (timeWait) {
                case 10:
                    PlayerService.gI().setPos(player, 335, 336, 0);
                    PlayerService.gI().playerMove(npc, 385, 336);
   ...
   private void resetSkill() {
    ...
    public void endChallenge() {
        ...
        if (player != null && player.zone != null && player.zone.map.mapId == ConstMap.DAI_HOI_VO_THUAT_129) {
            Util.setTimeout(() -> {
                ChangeMapService.gI().changeMapNonSpaceship(player, ConstMap.DAI_HOI_VO_THUAT_129, player.location.x, 408);
            }, 500);
        }
        if (boss != null) {
            boss.leaveMap();
