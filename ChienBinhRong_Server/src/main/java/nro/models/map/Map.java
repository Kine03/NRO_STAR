    ...
    public void initZone(int number, int maxPlayer) {
        ...
        switch (this.type) {
            ...
            case ConstMap.MAP_KHI_GAS_HUY_DIET:
                countZone = KhiGas.MAX_AVAILABLE;
                break;
            default:
                break;
        }
...
//                Logger.logException(Map.class, e, "Lỗi update map " + this.mapName);
    ...
    private void initItem() {
        for (Zone zone : zones) {
            ...
            switch (this.mapId) {
                case 21:
                    itemMap = new ItemMap(zone, 74, 1, 632, 326, -1);
                    break;
                case 22:
                    itemMap = new ItemMap(zone, 74, 1, 994, 345, -1);
                    break;
                case 23:
                    itemMap = new ItemMap(zone, 74, 1, 583, 230, -1);
                    break;
                case 42:
                    itemMap = new ItemMap(zone, 78, 1, 90, 288, -1);
                    break;
                case 43:
                    itemMap = new ItemMap(zone, 78, 1, 90, 264, -1);
                    break;
                case 44:
                    itemMap = new ItemMap(zone, 78, 1, 90, 288, -1);
                    break;
    ...
    public int yPhysicInTop(int x, int y) {
        ...
        // Kiểm tra phạm vi của y/SIZE và rX trước khi truy cập vào tileMap
        if (rX >= 0 && rX < tileMap[0].length && y / SIZE >= 0 && y / SIZE < tileMap.length) {
            if (isTileTop(tileMap[y / SIZE][rX])) {
                return y;
            }
        } else {
            // Trả về một giá trị hợp lệ hoặc xử lý ngoại lệ nếu vị trí không hợp lệ
            return 0;
        }

        // Vòng lặp kiểm tra các hàng tiếp theo
        for (int i = y / SIZE; i < tileMap.length; i++) {
            if (rX >= 0 && rX < tileMap[i].length) {
                if (isTileTop(tileMap[i][rX])) {
                    rY = i * SIZE;
                    break;
                }
            } else {
                break; // Thoát khỏi vòng lặp nếu rX không hợp lệ
            }
        }
        return rY;
    }
