...
public class TranhNgoc {
    ...
    public static final byte MIN_OPEN = 15;
    ...
    public byte getBalancedPhe() {
        int countCadic = playersCadic.size();
        int countFide = playersFide.size();
        if (countCadic > countFide) {
            return 2;
        }
        if (countFide > countCadic) {
            return 1;
        }
        return (byte) Util.nextInt(1, 2);
    }
        ...
        try {
            ...
            if (Util.canDoWithTime(player.lastTimeUpdateBallWar, 1000)) {
                ...
                if (player.zone != null && player.zone.map.mapId == ConstTranhNgocNamek.MAP_ID) {
                    try {
                        // Bỏ qua nếu là pet hoặc minipet
                        ...
                        if (!isTimeStartWar() || (!player.zone.getPlayersFide().contains(player) && !player.zone.getPlayersCadic().contains(player))) {
                            player.isHoldNamecBall = false; // thêm dòng này
