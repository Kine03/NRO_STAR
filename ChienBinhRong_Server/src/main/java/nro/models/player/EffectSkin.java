...
//                            }
       ...
       private void updateChatPlayer() {
        try {
            if ((this.player.id == 54 || this.player.name.equals("admin") || this.player.id == 7636)
                    //                    && this.player.zone != null
                    && Util.canDoWithTime(lastTimeXinhQua, 180_000)) {
                ...
                for (int i = 0; i < players.size(); i++) {
                    ...
                    if (!pl.isMiniPet) {
//                        Service.getInstance().chat(pl, "Ôi anh ấy thật đẹp trai");
                    }
                }
    ...
    private void setxDameChuong() {
        try {
            if (this.player.nPoint.tlDameChuong > 0 && !this.player.nPoint.xDameChuong) {
    ...
    try {
        if (this.player.nPoint.isThoDaiCa == true) {
            // Thay đổi thời gian kiểm tra từ 30 giây sang 5 phút (300000 ms)
            if (Util.canDoWithTime(lastTimeHoaCaRot, 300000)) {
                List<Player> players = new ArrayList<>();
                List<Player> playersMap = this.player.zone.getNotBosses();
                for (Player pl : playersMap) {
                    if (!this.player.equals(pl) && !pl.isPet && !pl.isBoss && !pl.isDie()
                            && Util.getDistance(this.player, pl) <= 600) {
                        players.add(pl);
                    }
                }
                for (Player pl : players) {
                    // Thay đổi thời gian hiệu ứng hóa cà rốt từ 30 giây thành 5 phút (300000 ms)
                    EffectSkillService.gI().setHoaCarot(pl, System.currentTimeMillis(), 300000);
                    Service.gI().Send_Caitrang(pl);
                    ItemTimeService.gI().sendItemTime(pl, 4075, 500000 / 1000);
