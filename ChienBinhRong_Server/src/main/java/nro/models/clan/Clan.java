...
public class Clan {
    ...
    public boolean bossbdkb;
    public long xongbossbdkb;
    public long xongbosskg;
    ...
    public void insert() {
        ...
        try ( Connection con = DBService.gI().getConnectionForClan();) {
            ps = con.prepareStatement("insert into clan_sv" + Manager.SERVER
                    + "(id, name, slogan, img_id, power_point, max_member, clan_point, level, members, thanhTichBDKB, thongTinLeader, thanhTichKhiGas) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?,?)");
            ps.setInt(1, this.id);
    ...
    public void update() {
        ...
        try {
            ps = con.prepareStatement("update clan_sv" + Manager.SERVER
                    ...
                    + "level = ?, members = ?, thanhTichBDKB = ?, thongTinLeader = ?, thanhTichKhiGas where id = ? limit 1");
    ...
    public void updatethanhTichKG(int clanId) {
        ...
        try {
            ps = con.prepareStatement("update clan_sv" + Manager.SERVER
                    + " set thanhTichKhiGas = ? where id = ? limit 1");
            ps.setString(1, topKhiGas);
            ps.setInt(2, clanId);
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
            Log.error(Clan.class, e, "ERROR KHI UPDATE THÀNH TÍCH BANG 2");
        } finally {
            try {
                ps.close();
            } catch (Exception e) {
