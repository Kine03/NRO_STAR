...
//                    Service.getInstance().regisAccount(_session, _msg);
                    Service.getInstance().sendThongBao(player, "Len Trang Chu Dang Ki Nha Ban!!!!");
                    break;
                case -101:
//                    try {
...
//                                SieuHangService.gI().startChallenge(player, pId);
                            ...
                            Item maydo = InventoryService.gI().findItemBagByTemp(player, 1449);
                ...
                int effId = _msg.reader().readShort();
                    ...
                    Resources.getInstance().loadEffectData(_session, effId,idT);
                    break;
                case -62:
                    if (player != null) {
...
//                            Service.getInstance().minigame_taixiu(player);
               ...
               case -20:
                ...
                case -15: // về nhà
                    if (player != null) {
                        if (player.zone.map.mapId == ConstTranhNgocNamek.MAP_ID) {
                            Service.getInstance().sendThongBao(player, "Bạn không thể về nhà cho đến khi kết thúc");
                            return;
                        }
...
//    }
    ...
    public void thongBaoLockKey(Session session) {
        Service.getInstance().sendThongBaoOK(session, "Vui lòng tải phiên bản chính thúc tại\bngocrongstar.online");
        return;
    }
...
//    }
    public void login2(Session session, Message msg) {
        Service.getInstance().sendThongBaoOK(session, "Lên trang chủ: đăng kí nha bạn và nhớ tham gia box ZALO nhận thông báo ưu đãi mới nhất!");
    }
...
//    player.inventory.ruby = 1500000000;
//}
        player.soDuVND = player.getSession().vndBar;
        
        player.DiemGt = player.getSession().referral_points;
        ...
        if (player.pet != null) {
            player.lvpet = (short) player.pet.lever;
            if(player.lvpet > 7){
                Item dns = ItemService.gI().createNewItem((short) 674, (player.lvpet - 7) * 50);
                InventoryService.gI().addItemBag(player, dns, 99999);
                InventoryService.gI().sendItemBags(player);
                Service.getInstance().sendThongBaoOK(player, "Bạn nhận được " + dns.getName() +" Do đã fix cấp đệ xuống còn 7");
                player.lvpet = 7;
                player.pet.setLever(7);
            }
        }
