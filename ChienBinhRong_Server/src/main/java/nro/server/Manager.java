...
import nro.lib.ArrTemplate.ArrHead2Frames;
...
public class Manager {
    ...
    public static short MAX_PER_IP = 2;
    public static int MAX_PLAYER = 1000;
    public static byte RATE_EXP_SERVER = 10;
    public static int EVENT_SEVER = 1;
    public static String DOMAIN = "NGOCRONGSTART.ONLINE";

    public static int danhquairoingoc;

    public static Timestamp timeSuKienDuaTop = Timestamp.valueOf("2025-09-07 12:00:00");

    public static String timeStartDuaTop = "12h00 ngày 07/09/2025";

    public static String timeEndDuaTop = "20h ngày 14/09/2025";

    public static String timeEndNhanGiai = "14/09/2025";

    public static String SERVER_NAME = "NGOCRONGSTART.ONLINE";
    ...
    public static final short[] wtl = {556, 558, 560};
    ...
    private void initRandomItem() {
        HONG_DAO_CHIN.add(20, ConstItem.CHU_GIAI);
        HONG_DAO_CHIN.add(20, ConstItem.CHU_AN);
        HONG_DAO_CHIN.add(20, ConstItem.CHU_KHAI);
        HONG_DAO_CHIN.add(20, ConstItem.CHU_PHONG);
        HONG_DAO_CHIN.add(20, ConstItem.HONG_NGOC);
...
//    }
    private void initMap() {
        ...
        // 🧠 Chỉ tạo 1 thread duy nhất để cập nhật tất cả map
        new Thread(() -> {
            try {
                while (!Maintenance.isRuning()) {
                    long st = System.currentTimeMillis();

                    // Cập nhật toàn bộ map và zone
                    for (nro.models.map.Map map : MAPS) {
                        for (Zone zone : map.zones) {
                            try {
                                ...
                                // Ghi log lỗi nếu cần
                                // Log.error("Error update zone in map " + map.mapName, e);
                            }
                        }
                    }

                    // Giữ nhịp update 1s/lần
...
//                    dataObject.clear();
            ...
            Log.success("Load mod reward thành công (" + MOB_REWARDS.size() + ")");
            //load mob template
...
//            System.exit(0);
    ...
    public void loadProperties() throws IOException {
        ...
        if ((value = properties.get("server.maxperip")) != null) {
            MAX_PER_IP = Short.parseShort(String.valueOf(value));
        }
    ...
    private int[][] readTileMap(int mapId) {
        int[][] tileMap = null;
        try {
            DataInputStream dis = new DataInputStream(new FileInputStream("resources/louisgoku/map/tile_map_data/" + mapId));
            int w = dis.readByte();
            int h = dis.readByte();
            tileMap = new int[h][w];
