...
//        }
        if (!player.getSession().actived) {
            ...
            if (player.isAdmin() || Util.canDoWithTime(player.lastTimeChatGlobal, 60000)) {
                if (player.isAdmin() || player.nPoint.power >= 20) {
                ...
                Service.getInstance().sendThongBao(player, "Không thể chat thế giới lúc này, vui lòng đợi "
                        + TimeUtil.getTimeLeft(player.lastTimeChatGlobal, 60));
            }
        }
    }
