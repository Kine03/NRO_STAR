...
public class GiftService {
    ...
    public void use(Player player, String code) {
        ...
        try {
            ...
            try {
                ...
                int limit = res.getInt("limit"); // Giới hạn lượt nhập
                int usedCount = getUsedGiftCodeCount(id); // Số lượt đã sử dụng
                ...
                if (status == 1) {
                    ...
                    return;
                } else if (limit > 0 && usedCount >= limit) {
                    Service.getInstance().sendThongBaoOK(player, "Mã quà tặng này đã đạt giới hạn lượt nhập.");
                    return;
                ...
                // Giảm limit nếu là loại sử dụng một lần
                if (limit > 0 && (type == 1 || usedCount == 0)) {
                    res.updateInt("limit", limit - 1); // Giảm limit
                    res.updateRow();
                }
            } finally {
                res.close();
    ...
    private int getUsedGiftCodeCount(int giftCodeId) {
        try {
            PreparedStatement stmt = DBService.gI().getConnectionForGame().prepareStatement("SELECT COUNT(*) AS count FROM `gift_code_histories` WHERE `gift_code_id` = ?;", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
            stmt.setInt(1, giftCodeId);
            ResultSet res = stmt.executeQuery();
            try {
                if (res.first()) {
