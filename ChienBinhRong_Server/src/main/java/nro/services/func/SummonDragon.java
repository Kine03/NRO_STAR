...
public class SummonDragon {
    ...
    public static final String ICE_SHENRON_SAY
            ...
            + "\n 3) Sao Pha Lê Siêu Cấp Sức Đánh "
            + "\n 4) Sao Pha Lê Siêu Cấp HP "
            + "\n 5) Sao Pha Lê Siêu Cấp KI "
            + "\n 6) Biến Hình Lv1 ";
    ...
    private final int timeResummonShenron = 60000;
    ...
    public void summonShenron(Player pl) {
        if (pl.zone.map.mapId == 0 || pl.zone.map.mapId == 7 || pl.zone.map.mapId == 14) {
            if (checkShenronBall(pl, DRAGON_SHENRON)) {
                if (isShenronAppear || isIcecShenronAppear) {
                    Service.getInstance().sendThongBao(pl, "Rồng thiên đã xuất hiện vui lòng đợi");
                    return;
                }
    ...
    public void summonIceShenron(Player pl) {
        if (pl.zone.map.mapId == 5) {
            if (checkShenronBall(pl, DRAGON_ICE_SHENRON)) {
                if (isIcecShenronAppear || isShenronAppear) {
                    Service.getInstance().sendThongBao(pl, "Rồng thiên đã xuất hiện vui lòng đợi");
                    return;
                }
                if (Util.canDoWithTime(lastTimeShenronAppeared, timeResummonShenron)) {
                    ...
                    activeDragonICENew(pl, true, (byte) 59);
                    sendWhishesShenron(pl);
                } else {
                    int timeLeft = (int) ((timeResummonShenron - (System.currentTimeMillis() - lastTimeShenronAppeared)) / 1000);
                    Service.getInstance().sendThongBao(pl, "Vui lòng đợi " + (timeLeft < 60 ? timeLeft + " giây" : timeLeft / 60 + " phút") + " nữa");
    ...
    public void activeDragonICENew(Player pl, boolean appear, byte eff) {
        Message msg;
        try {
            msg = new Message(Cmd.CALL_DRAGON);
            msg.writer().writeByte(appear ? 0 : (byte) 1);
    ...
    public void confirmWish() {
        if (playerSummonShenron == null) {
            Log.error("Player is null in SummonDragon.confirmWish()");
            return;
        }
        switch (this.menuShenron) {
            case ConstNpc.SHENRON_1_1:
                switch (this.select) {
                    case 0: //quần đang mặc lên 1 cấp
                        ...
                        if (itemQuan.isNotNullItem()) {
                            ...
                            if (level < 7) {
                                ...
                                Service.gI().sendThongBao(playerSummonShenron, "Quần của ngươi đã đạt cấp tối đa");
                                reOpenShenronWishes(playerSummonShenron);
                                return;
                            }
                        } else {
                            Service.gI().sendThongBao(playerSummonShenron, "Ngươi hiện tại có mang quần xịt đâu");
                            reOpenShenronWishes(playerSummonShenron);
                            return;
                        }
                        break;
                    case 1: //găng tay đang đeo lên 1 cấp
                        Item itemGang = this.playerSummonShenron.inventory.itemsBody.get(2);
                        if (itemGang.isNotNullItem()) {
                            int level = 0;
                            ...
                            if (level < 7) {
                                if (level == 0) {
                                    itemGang.itemOptions.add(new ItemOption(72, 1));
                                }
                                for (ItemOption io : itemGang.itemOptions) {
                        ...
                        break;
                    case 2: //giày tay đang mang lên 1 cấp
                        Item itemGiay = this.playerSummonShenron.inventory.itemsBody.get(3);
                        if (itemGiay.isNotNullItem()) {
                            int level = 0;
                            ...
                            if (level < 7) {
                                if (level == 0) {
                                    itemGiay.itemOptions.add(new ItemOption(72, 1));
                                }
                                for (ItemOption io : itemGiay.itemOptions) {
                                ...
                                Service.getInstance().sendThongBao(playerSummonShenron, "Giày thúi của ngươi đã đạt cấp tối đa");
                                reOpenShenronWishes(playerSummonShenron);
                                return;
                            }
                        } else {
                    ...
                    case 4:
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            Item item2082 = ItemService.gI().createNewItem((short) (2082), 1);
                            InventoryService.gI().addItemBag(playerSummonShenron, item2082, 9999);
            ...
            case ConstNpc.SHENRON_1_2:
                switch (this.select) {
                    case 0: //+500 hồng ngọc
                        this.playerSummonShenron.inventory.ruby += 500;
                        PlayerService.gI().sendInfoHpMpMoney(this.playerSummonShenron);
                        break;
                    case 1: //Quần tay đệ lên 1 cấp
                        if (this.playerSummonShenron != null && this.playerSummonShenron.pet != null) {
                            Item item = this.playerSummonShenron.pet.inventory.itemsBody.get(1);
                            if (item.isNotNullItem()) {
                                ...
                                if (level < 7) {
                                    ...
                                    Service.getInstance().sendThongBao(playerSummonShenron, "Quần của đệ ngươi đã đạt cấp tối đa");
                                    reOpenShenronWishes(playerSummonShenron);
                                    return;
                                }
                            } else {
                                Service.getInstance().sendThongBao(playerSummonShenron, "Đệ ngươi hiện tại có mặc quần đâu");
                                reOpenShenronWishes(playerSummonShenron);
                                return;
                            }
                        } else {
                    ...
                    case 2: //găng tay đệ lên 1 cấp
                        ...
                        break;
                    case 3: //giày tay đệ lên 1 cấp
                        if (this.playerSummonShenron != null && this.playerSummonShenron.pet != null) {
                            Item item = this.playerSummonShenron.pet.inventory.itemsBody.get(3);
                            if (item.isNotNullItem()) {
                                int level = 0;
                                for (ItemOption io : item.itemOptions) {
                                    if (io.optionTemplate.id == 72) {
                                ...
                                if (level < 7) {
                                    ...
                                    Service.getInstance().sendThongBao(playerSummonShenron, "Giày thúi của đệ ngươi đã đạt cấp tối đa");
                                    reOpenShenronWishes(playerSummonShenron);
                                    return;
                                }
                            } else {
                                Service.getInstance().sendThongBao(playerSummonShenron, "Đệ ngươi hiện tại có mang giày đâu");
                                reOpenShenronWishes(playerSummonShenron);
                                return;
                            }
                        } else {
            ...
            case ConstNpc.BLACK_SHENRON:
                ...
                break;
            case ConstNpc.ICE_SHENRON://Ngọc rồng băng
                switch (this.select) {
                    case 0:
                    ...
                    case 2:
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            Item it = ItemService.gI().createNewItem((short) 1486);
                            it.itemOptions.add(new ItemOption(50, 4));
                    ...
                    case 3:
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            Item it = ItemService.gI().createNewItem((short) 1484);
                            it.itemOptions.add(new ItemOption(77, 7));
                            InventoryService.gI().addItemBag(playerSummonShenron, it, 0);
                            InventoryService.gI().sendItemBags(playerSummonShenron);
                        } else {
                            Service.getInstance().sendThongBao(playerSummonShenron, "Hành trang đã đầy");
                    ...
                    case 4:
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            Item it = ItemService.gI().createNewItem((short) 1485);
                            it.itemOptions.add(new ItemOption(103, 7));
                            InventoryService.gI().addItemBag(playerSummonShenron, it, 0);
                            InventoryService.gI().sendItemBags(playerSummonShenron);
                        } else {
                            Service.getInstance().sendThongBao(playerSummonShenron, "Hành trang đã đầy");
                    ...
                    case 5:
                        ...
                        if (InventoryService.gI().getCountEmptyBag(playerSummonShenron) > 0) {
                            ...
                            Item skillBienHinh = ItemService.gI().createNewItem((short) (gender == ConstPlayer.TRAI_DAT ? 1533
                                    : gender == ConstPlayer.NAMEC ? 1528 : 1523));
                            skillBienHinh.itemOptions.add(new ItemOption(30, 1));
                            InventoryService.gI().addItemBag(playerSummonShenron, skillBienHinh, 0);
                            InventoryService.gI().sendItemBags(playerSummonShenron);
                        } else {
                            Service.getInstance().sendThongBao(playerSummonShenron, "Hành trang đã đầy");
                            reOpenShenronWishes(playerSummonShenron);
