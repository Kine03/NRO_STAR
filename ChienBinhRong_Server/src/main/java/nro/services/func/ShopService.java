...
public class ShopService {
    ...
    private static final int COST_LOCK_GOLD_BAR = 1000000000;
    ...
    private ItemShop getItemShop(Player player, int shopId, int tempId) {
        ...
        switch (shopId) {
            ...
            case ConstNpc.SHOP_URON_0:
                ...
                break;
            case ConstNpc.SHOP_QUY_LAO_KAME_2:
                shop = getShop(player, ConstNpc.QUY_LAO_KAME, 2, -1);
                break;
            ...
            case ConstNpc.TRUNGTHU:
                shop = getShop(player, ConstNpc.TRUNG_THU, 0, -1);
                break;
            case ConstNpc.SHOP_xinbato:
                shop = getShop(player, ConstNpc.xinbato, 0, -1);
                break;
            case ConstNpc.SHOP_OSIN:
                shop = getShop(player, ConstNpc.OSIN, 2, -1);
                break;
        }
        if (shop != null) {
            ...
            if (shop.npcId == npcId && shop.shopOrder == order) {
                for (TabShop tabShop : shop.tabShops) {
                    for (ItemShop item : tabShop.itemShops) {
                        switch (item.temp.id) {
                            case 5178:// hành trang
                                item.gem = (player.inventory.itemsBag.size() - 19) * 108;
                                break;
                            case 518:// rương đồ
...
//                    System.out.println("shopId: " + shopId);
    ...
    private void buyItemShopNormal(Player player, ItemShop is) {
        if (is != null) {
            ...
            if (is.temp.id == 517 && player.inventory.itemsBag.size() >= 108) {
                Service.getInstance().sendThongBao(player, "Đã đạt mức tối đa");
                Service.getInstance().sendMoney(player);
                return;
            }
            if (is.temp.id == 518 && player.inventory.itemsBox.size() >= 108) {
                Service.getInstance().sendThongBao(player, "Đã đạt mức tối đa");
                Service.getInstance().sendMoney(player);
                return;
            }
...
//                        Service.getInstance().sendThongBao(player, "Đổi thành công " + is.temp.name);
                    ...
                    case ConstNpc.SHOP_SU_KIEN_TET:
                        ...
                        break;
                    default:
                        InventoryService.gI().addItemBag(player, ItemService.gI().createItemFromItemShop(is), 99999);
                        InventoryService.gI().sendItemBags(player);
    ...
    public void openBoxItemReward(Player player) {
        ...
        try {
            ...
            for (Item item : player.getSession().itemsReward) {
                msg.writer().writeShort(item.template.id);
                msg.writer().writeUTF("Phần Thưởng Đua Top");
                List<ItemOption> itemOptions = item.getDisplayOptions();
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (item.template.id == 457) {
                ...
                Input.gI().createFormBanThoiVangKhoa(pl);
                return;
            } else {
