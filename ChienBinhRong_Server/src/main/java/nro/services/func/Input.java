...
public class Input {
    ...
    public static final int TRADE_RUBY = 512;
    public static final int BAN_THOI_VANG = 511;
    public static final int BAN_THOI_VANG_KHOA = 514;
    public static final int BAN_BRICS = 513;
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case TRADE_RUBY:
                    ...
                    if (player.soDuVND < cuantity) {
                        Service.getInstance().sendThongBao(player, "Số dư không đủ vui lòng nạp thêm!");
                    } else {
                        PlayerDAO.subVND2(player, cuantity);
                        player.soDuVND -= cuantity;
                        int rubyBonus = 0;
                        if (cuantity >= 10_000) {
                            rubyBonus = cuantity / 10; // 10%
                        }
                        player.inventory.ruby += cuantity + rubyBonus;

                        Service.getInstance().sendMoney(player);
                        Service.getInstance().sendThongBao(player, "Đã đổi thành công " + (cuantity + rubyBonus) + " hồng ngọc");
                    }
                    break;
                case CHANGE_NAME:
                    if (player != null) {
...
//                    }
                ...
                case TAI_taixiu:
                    ...
                    try {
                        if (sotvxiu1 >= 10 && sotvxiu1 <= 500) {
                            Item tv = InventoryService.gI().findItemBag(player, 457);
                            if (tv != null && tv.quantity >= sotvxiu1) {
                                tv.quantity -= sotvxiu1;
                                if (tv.quantity == 0) {
                                    InventoryService.gI().removeItemBag(player, tv);
                                }
                            ...
                            Service.getInstance().sendThongBao(player, "Cược ít nhất 10 thỏi vàng và nhiều nhất 500");
                        }
                ...
                case XIU_taixiu:
                    ...
                    try {
                        if (sotvxiu2 >= 10 && sotvxiu2 <= 500) {
                            Item tv = InventoryService.gI().findItemBag(player, 457);
                            if (tv != null && tv.quantity >= sotvxiu2) {
                                tv.quantity -= sotvxiu2;
                                if (tv.quantity == 0) {
                                    InventoryService.gI().removeItemBag(player, tv);
                                }
                ...
                case BAN_THOI_VANG:
                    ...
                    break;
                case BAN_THOI_VANG_KHOA:
                    long soLuong2 = Long.parseLong(text[0]);
                    Item thoiVangKhoa = InventoryService.gI().findItemBagByTemp(player, (short) 2011);
                    if (soLuong2 < 0) {
                        Service.getInstance().sendThongBao(player, "Đã bán " + soLuong2 + " bãi cứt" + " thu được 1" + " vàng");
                        return;
                    }
                    if (soLuong2 <= thoiVangKhoa.quantity) {
                        long goldNhanDuoc = soLuong2 * 500000000;
                        long soGoldAll = goldNhanDuoc + player.inventory.gold;
                        if (soGoldAll <= player.inventory.getGoldLimit()) {
                            player.inventory.gold += (soLuong2 * 500000000);
                            InventoryService.gI().subQuantityItemsBag(player, thoiVangKhoa, (int) soLuong2);
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendMoney(player);
                            Service.getInstance().sendThongBao(player, "Đã bán " + soLuong2 + " " + thoiVangKhoa.getName() + " thu được " + Util.numberToMoney(goldNhanDuoc) + " vàng");
                        } else {
                            Service.getInstance().sendThongBao(player, "Số vàng sau khi bán vượt quá số vàng có thể lưu trữ");
                        }
                    } else {
                ...
                case BAN_BRICS:
                    ...
                    Item thoiVang1 = InventoryService.gI().findItemBagByTemp(player, (short) 457);
                    if (soLuong1 < 0) {
                        Service.getInstance().sendThongBao(player, "Đã bán " + soLuong1 + " bãi cứt" + " thu được 1" + " vàng");
                        return;
                    }
                    if (soLuong1 <= thoiVang1.quantity) {
                        long goldNhanDuoc = soLuong1 * 1_400_000_000;
                        long soGoldAll = goldNhanDuoc + player.inventory.gold;
                        if (soGoldAll <= player.inventory.getGoldLimit()) {
                            player.inventory.gold += (soLuong1 * 1_400_000_000);
                            InventoryService.gI().subQuantityItemsBag(player, thoiVang1, (int) soLuong1);
                            InventoryService.gI().sendItemBags(player);
                            Service.getInstance().sendMoney(player);
                            Service.getInstance().sendThongBao(player, "Đã bán " + soLuong1 + " " + thoiVang1.getName() + " thu được " + Util.numberToMoney(goldNhanDuoc) + " vàng");
                        } else {
                            Service.getInstance().sendThongBao(player, "Số vàng sau khi bán vượt quá số vàng có thể lưu trữ");
                        }
                    } else {
                ...
                case CON_SO_MAY_MAN_NGOC:
                    ...
                    if (CSMM >= MiniGame.gI().MiniGame_S1.min && CSMM <= MiniGame.gI().MiniGame_S1.max && MiniGame.gI().MiniGame_S1.second > 10) {
                        MiniGame.gI().MiniGame_S1.newData(player, CSMM, 1);
                    }
                    break;
    ...
    public void createFormTradeRuby(Player pl) {
        createForm(pl, TRADE_RUBY, "Tỉ lệ quy đổi: 1000 vnđ = 1000 Hồng ngọc \n Số dư hiện tại: " + pl.soDuVND, new SubInput("Số lượng", NUMERIC));
    }
    ...
    public void createFormChangeName(Player pl) {
    ...
    public void createFormBanThoiVangKhoa(Player pl) {
        createForm(pl, BAN_THOI_VANG_KHOA, "Bạn muốn bán bao nhiêu [Thỏi vàng] ?", new SubInput("Số lượng", NUMERIC));
    }

    public void createFormBanBrics(Player pl) {
        createForm(pl, BAN_BRICS, "Bạn muốn bán bao nhiêu [BRICS] ?", new SubInput("Số lượng", NUMERIC));
    }
    ...
    public void createFormConSoMayMan_Gem(Player pl) {
        createForm(pl, CON_SO_MAY_MAN_NGOC, "Hãy chọn 1 số từ 0 đến 99 giá 1 Thỏi vàng", new SubInput("Số bạn chọn", NUMERIC));
    }

    public void createFormConSoMayMan_Gold(Player pl) {
        createForm(pl, CON_SO_MAY_MAN_VANG, "Hãy chọn 1 số từ 0 đến 99 1 thỏi vàng", new SubInput("Số bạn chọn", NUMERIC));
    }
