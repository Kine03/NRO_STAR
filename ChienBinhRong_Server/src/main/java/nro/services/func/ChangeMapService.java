...
//                    Logger.logException(MapService.class, e);
    ...
    public void changeMapInYard(Player pl, int mapId, int zoneId, int x) {
        ...
        if (zoneJoin == null || zoneJoin.map == null) {
            Service.gI().sendThongBao(pl, "Không thể vào bản đồ lúc này.");
            return;
        }
...
//        spaceShipArrive(player, (byte) 1, DEFAULT_SPACE_SHIP);
    ...
    public Zone checkMapCanJoin(Player player, Zone zoneJoin) {
        ...
        if (zoneJoin != null) {
            switch (zoneJoin.map.mapId) {// cần sữa
                case 1: //đồi hoa cúc
                case 8: //đồi nấm tím
                case 15: //đồi hoang
...
//                     Service.getInstance().sendThongBaoOK(player, "Hãy thách đấu với Yanjirô mới có thể qua");
//                        return null;
//                    }
//                    break;
                case 6: //đông karin
