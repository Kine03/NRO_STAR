...
//    }
    ...
    public List<ItemMap> getRewardItems(Player player, Mob mob, int x, int yEnd) {
        ...
        MobReward mobReward = Manager.MOB_REWARDS.get(mob.id);
        if (mobReward != null) {
...
//                        initActivationOption(player, itemMap);
                ...
                if (MapService.gI().isMapBangHoiBongTai(player.zone.map)) {//mhvt, mvbt   
                    if (mob.tempId == 81) {
                    ...
                    if (mob.tempId == 80) {
                        if (Util.isTrue(10, 100)) {
                            // Random giữa 933 và 934
//                        int[] ids = {933, 934};
//                        int itemId = ids[Util.nextInt(0, ids.length - 1)];
                ...
                if (MapService.gI().isMapBongTai3(player.zone.map)) {//mvbt3   
                        if (Util.isTrue(30, 100)) {
                            ItemMap itemMap = new ItemMap(mob.zone, 1130, 1, x, yEnd, player.id);
                            list.add(itemMap);
                        }                    
                }
                if (MapService.gI().isMapTuongLai(player.zone.map)) {//item cskb                   
                    if (player.itemTime.isUseMayDo) {
                        if (Util.isTrue(20, 200)) {
                            // Random giữa 380 và 1499
                            int[] ids = {380, 1499};
                            int itemId = ids[Util.nextInt(0, ids.length - 1)];
                            ItemMap itemMap = new ItemMap(mob.zone, itemId, 1, x, yEnd, player.id);
                            list.add(itemMap);
                        }
                    }
                }
                if (MapService.gI().isMapPhapSu(player.zone.map)) {//item đá pháp sư
                    if (player.itemTime.isUseBanhChung) {
                        if (Util.isTrue(10, 100)) {
                            ItemMap itemMap = new ItemMap(mob.zone, 1379, 1, x, yEnd, player.id);
                            list.add(itemMap);
                        }
                    }
                }
                if (MapService.gI().isMapDaCam(player.zone.map)) {//item đá cam
                    if (player.itemTime.isUseCuongbao) {
                        if (Util.isTrue(10, 100)) {
                            ItemMap itemMap = new ItemMap(mob.zone, 1450, 1, x, yEnd, player.id);
                            list.add(itemMap);
                        }
                    }
                }
                ...
                if (mob.tempId == ConstMob.HIRUDEGARN) {
                    ...
                    ItemMap trungMabu = new ItemMap(mob.zone, (short) 1459, 1, x, yEnd, player.id);
                    list.add(trungMabu);

                    for (int i = 0; i < 3; i++) {
                        int itemID = rd.next();
                        // Kiểm tra nếu itemID là 1522 thì rơi 5 cái
                        if (itemID == 1522) {
                            for (int j = 0; j < 3; j++) { // Rơi 5 cái item 1522
                                ItemMap itemMap = new ItemMap(mob.zone, 1522, 1, x + Util.nextInt(-50, 50), yEnd, player.id);
                                list.add(itemMap);
                            }
                        } else {
                ...
                /// point + khi tiêu diệt máy đo
                if (mob.tempId == ConstMob.MAY_DO_SUC_MANH) {
                    player.diemmaydam += 1;
                }
                //roi hoa hong
                ...
                int slsk = 0;
                if (MapService.gI().isMapNguHanhSon(mapid) || MapService.gI().isMapCold(player.zone.map) || mapid == 212) {
                    slsk = 2;
                }
//                if (Util.isTrue(1, 100000)) {
//                    ItemMap itemMap = new ItemMap(mob.zone, Util.nextInt(886, 887), slsk, x, yEnd, player.id);
//                    list.add(itemMap);
//                }
//                if (Util.isTrue(8, 100000)) {
//                    ItemMap itemMap = new ItemMap(mob.zone, Util.nextInt(888, 889), slsk, x, yEnd, player.id);
//                    list.add(itemMap);
//                }
                //sk ngu hanh son
...
//                    }
//                }
//                if (MapService.gI().isMapThucvat(player.zone.map)) {
//                    if (Util.isTrue(10, 100) && player.isPl()) {
//                        ItemMap itemMap = new ItemMap(mob.zone, ConstItem.BI_KIP_TUYET_KY, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                    if (Util.isTrue(30, 100) && player.isPl()) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 1291, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
                if (mapid == 155 && player.setClothes.setts == 5) {
                    if (Util.isTrue(20, 100)) {
                        ItemMap itemMap = new ItemMap(mob.zone, 1066, 1, x, yEnd, player.id);
                        list.add(itemMap);

                    }
                    if (Util.isTrue(20, 100)) {
                        ItemMap itemMap = new ItemMap(mob.zone, 1067, 1, x, yEnd, player.id);
                        list.add(itemMap);
                    }
                    if (Util.isTrue(20, 100)) {
                        ItemMap itemMap = new ItemMap(mob.zone, 1068, 1, x, yEnd, player.id);
                        list.add(itemMap);
                    }
                    if (Util.isTrue(15, 100)) {
                        ItemMap itemMap = new ItemMap(mob.zone, Util.nextInt(1069, 1070), 1, x, yEnd, player.id);
                        list.add(itemMap);
                    }
                }
//                if (mapid == 212 && player.setClothes.setan == 5) {
//                    if (Util.isTrue(20, 100)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 1403, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//
//                    }
//                    if (Util.isTrue(20, 100)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 1404, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                    if (Util.isTrue(15, 100)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 1405, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
                //color doTL
                if (MapService.gI().isMapCold(player.zone.map)) {
                    ...
                    if (Util.isTrue(1, 50000)) {
                        ItemMap itemMap = Util.ratiItem(mob.zone, itemDoTL[randomDoTL], 1, x, yEnd, player.id);
                        list.add(itemMap);
                    } else if (Util.isTrue(1, 100000)) {
                        ItemMap itemMap = Util.ratiItem(mob.zone, gangTL[randomGangTL], 1, x, yEnd, player.id);
                        list.add(itemMap);
                    }
                }
                //rừng nguyên thủy
...
//                        List<Integer> item = Arrays.asList(674, 1450);
//                        ItemMap itemMap = new ItemMap(mob.zone, Util.getRandomList(item), Util.nextInt(3, 5), x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
//                if (MapService.gI().trungthu(mapid)) {
//                    if (Util.isTrue(2, 100)) {
//                        List<Integer> item = Arrays.asList(1557, 1557);
//                        ItemMap itemMap = new ItemMap(mob.zone, Util.getRandomList(item), 2, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
//                if (Util.isTrue(2, 100)) {
//                    List<Integer> item = Arrays.asList(1557, 1557);
//                    ItemMap itemMap = new ItemMap(mob.zone, Util.getRandomList(item), 1, x, yEnd, player.id);
//                    list.add(itemMap);
//                }
//                if (MapService.gI().isMapBanDoKhoBau(mapid)) {
//                    if (Util.isTrue(2, 100)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 674, Util.nextInt(3, 5), x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
                if (MapService.gI().isMapKhiGas(mapid)) {
                    if (Util.isTrue(2, 100)) {
                        List<Integer> item = Arrays.asList(674, 1450);
                        ItemMap itemMap = new ItemMap(mob.zone, Util.getRandomList(item), Util.nextInt(3, 5), x, yEnd, player.id);
                        list.add(itemMap);
                ...
                if (MapService.gI().isMapcdrd(mapid)) {
                    if (Util.isTrue(2, 100)) {
                        List<Integer> item = Arrays.asList(674, 1450);
                        ItemMap itemMap = new ItemMap(mob.zone, Util.getRandomList(item), Util.nextInt(3, 5), x, yEnd, player.id);
                        list.add(itemMap);
...
//                if (mapid == 212) {
//                    if (Util.isTrue(1, 200)) {
//                        ItemMap itemMap = new ItemMap(mob.zone, 674, 1, x, yEnd, player.id);
//                        list.add(itemMap);
//                    }
//                }
...
//                }
    ...
    public void initActivationOption(int gender, int type, List<ItemOption> list) {
        if (type <= 4) {
            ...
            if (idOption[1] == 140) {
                param = 300;
            } else if (idOption[1] == 136 || idOption[1] == 143) {
                param = 150;
            } else {
                param = 100;
            }
            list.add(new ItemOption(idOption[1], param)); //hiệu ứng set
