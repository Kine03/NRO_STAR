...
//        }
    ...
    public List<Zone> getMapCapsule(Player pl) {
        ...
        // Kiểm tra bản đồ trước khi di chuyển
        if (pl.mapBeforeCapsule != null
                ...
                && !isMapTuongLai(pl.mapBeforeCapsule.map)) {
            addListMapCapsule(pl, list, pl.mapBeforeCapsule);
        }
        // Các map capsun chỉ hiển thị khi nhiệm vụ đã hoàn thành
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_1_0) {
            addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 21 + pl.gender, 0));
        }
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_8_4) {
            addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 47, 0));
        }
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_11_0) {
            addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 45, 0));
        }
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_4_0) {
        ...
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_17_0) {
            addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 19, 0));
        }
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_22_0) {
        ...
        if (TaskService.gI().getIdTask(pl) >= ConstTask.TASK_31_0) {
            addListMapCapsule(pl, list, getZoneJoinByMapIdAndZoneId(pl, 154, 0));
        }
        return list;
    }
    ...
    public boolean trungthu(int mapId) {
        return mapId == 212 || mapId >= 105 && mapId <= 110 || mapId >= 122 && mapId <= 124;
    }
    ...
    public boolean isMapZenoWar21H(int mapId) {
        return mapId == 219;
    }
    ...
    public boolean isMapPhapSu(Map map) {
        int mapId = map.mapId;
        return mapId >= 214 && mapId <= 216;
    }

    public boolean isMapDaCam(Map map) {
        int mapId = map.mapId;
        return mapId >= 217 && mapId <= 218;
    }

    boolean isMapThucvat(Map map) {
        int mapId = map.mapId;
        return mapId >= 160 && mapId <= 164;
    }
    ...
    boolean isMapBangHoiBongTai(Map map) {
        int mapId = map.mapId;
        return mapId == 153;
    }

    boolean isMapBongTai3(Map map) {
        int mapId = map.mapId;
