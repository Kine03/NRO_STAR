...
//        }
        ...
        if (item.template.id == 517) {
            ...
            if (player.inventory.itemsBox.size() < 108) {
    ...
    private boolean isItemIncremental(Item item) { //item cộng dồn số lượng
        if (item.template.id == 454 || item.template.id == 921 || item.template.id == 1015) {
            return false;
        }
        switch (item.template.type) {
            ...
            if (index >= 0 && index <= player.inventory.itemsBag.size()) {
                ...
                if (itemThrow.isNotNullItem()) {
                    if (itemThrow.template.id != 457
                            && itemThrow.template.id != 1579
                            && itemThrow.template.id != 1580
                            && itemThrow.template.id != 1581
                            && itemThrow.template.id != 1582
                            && itemThrow.template.id != 1583
                            && itemThrow.template.id != 1584) {
...
//        itemMap.options = itemThrow.itemOptions;
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        if (item.isNotNullItem()) {
            if (type >= 0 && type <= 5 || type == 32 || type == 11 || type == 23 || type == 24 || type == 72 || type == 21 || type == 35 || type == 99) {
                if (item.template.gender == player.gender || item.template.gender == 3) {
                    ...
                    if (forPet) {
                    ...
                    } else if (player.isPet && type > 6 && type != 32 && type != 35) {
                        Service.getInstance().sendThongBaoOK(player.isPet ? ((Pet) player).master : player, "Trang bị không phù hợp! 1");
                        ...
                        if (item.template.type == 99) {
                            Service.getInstance().removeTitle(player);
                            Service.getInstance().sendTitle(player);
                        }
                    } else {
                        Service.getInstance().sendThongBaoOK(player.isPet ? ((Pet) player).master : player, "Sức mạnh không đủ yêu cầu!");
                    }
                } else {
                    Service.getInstance().sendThongBaoOK(player.isPet ? ((Pet) player).master : player, "Trang bị không phù hợp! 2");
                }
            } else {
                Service.getInstance().sendThongBaoOK(player.isPet ? ((Pet) player).master : player, "Trang bị không phù hợp! 3");
            }
        }
        return sItem;
    }
    ...
    public void itemBagToBody(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            if (item.template.id == 1285 || item.template.id == 1287 || item.template.id == 1289) {
                player.newSkill.setTypePaint((byte) 2);
            }
            if (item.template.id == 1286 || item.template.id == 1288 || item.template.id == 1290) {
                player.newSkill.setTypePaint((byte) 3);
            }
    ...
    public void itemBodyToBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            if (item.template.id == 1285 || item.template.id == 1287 || item.template.id == 1289) {
                player.newSkill.setTypePaint((byte) -1);
            }
            if (item.template.id == 1286 || item.template.id == 1288 || item.template.id == 1290) {
                player.newSkill.setTypePaint((byte) -1);
            }
    ...
    public void itemBoxToBodyOrBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (!done) {
                if (addItemBag(player, item, 99999)) {
                    if (item.quantity == 0) {
                        Item sItem = ItemService.gI().createItemNull();
    ...
    public void itemBagToBox(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (addItemBox(player, item, 99999)) {
                if (item.quantity == 0) {
                    Item sItem = ItemService.gI().createItemNull();
