...
public class OpenPowerService {
    ...
    public boolean openPowerBasic(Player player) {
        ...
        if (curLimit < NPoint.MAX_LIMIT) {
            if (player.nPoint.limitPower > 9 && !player.setClothes.godClothes) {
                if (!player.isPet) {
                    Service.getInstance().sendThongBao(player, "Cần Mặc Set Thần Linh Để Mở");
                } else {
                    Service.getInstance().sendThongBao(((Pet) player).master, "Cần Mặc Set Thần Linh Để Mở");
                }
                return false;
            }
